...
import { sendValidationAccount, sendInvalidToken, sendConfirmationEmail } from "../email/email";
...
export const verifyMail = async (req :Request, res: Response) =>  {
        try {
            ...
            if (!user) {
                res.status(400).json({ message: "Token déjà validé ou inexistant.", status: 400 });
                return;
            }
            const decoded = jwt.verify(token, process.env.SECRET, { ignoreExpiration: true }) as { email: string; exp: number };
        ...
        res.status(500).json({ message: "Erreur interne du serveur", status: 500 });
        return
        }
    }
